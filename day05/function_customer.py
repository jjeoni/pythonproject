...
def customer_input(custlist):
    ...
    while True:
        ...
        if check == 0:
            break
        print('중복되는 이메일이 존재합니다.')
        
    while True:
        customer['birthyear'] = input('출생년도(4자리) : ')
        if len(customer['birthyear']) == 4 and customer['birthyear'].isnumeric():
            break
